...
<table id="identitas" cellspacing="0">
    ...
    <tr>
        ...
        <td>
            <span>: Tahfidzul Qur'an Lil Bana'at</span>
        </td>
        <td>
            <span>SEMESTER</span>
        </td>
    ...
    <tr>
        ...
        <td >
            <span >: Komp. Perumahan Ar-Rayhan  Rt.01 Talang Belido, <br> &nbsp; Sungai Gelam, Muaro Jambi
            </span>
        </td>
        <td>
            <span>TAHUN</span>
...
<table id="nilai">
    ...
    <tr>
        <th rowspan="{{count($nilaiDiniyahTeori)+1}}" style="text-align: center">
            <span>I</span>
        </th>
        <th rowspan="{{count($nilaiDiniyahTeori)+1}}" style="text-align: center">
    ...
    @foreach($nilaiDiniyahTeori as $nd)
    <tr>
    ...
    @endforeach
    @endif
    @if($nilaiBahasaTeori != null)
    <tr>
        <th rowspan="{{count($nilaiBahasaTeori)+1}}" style="text-align: center">
            <span>II</span>
        </th>
        <th rowspan="{{count($nilaiBahasaTeori)+1}}" style="text-align: center">
    ...
    @foreach($nilaiBahasaTeori as $nb)
    <tr>
        <td>{{$loop->iteration}}</td>
    ...
    <tr>
        <th rowspan="{{count($nilaiDiniyahPraktek)+1}}" style="text-align: center">
            <span>I</span>
        </th>
        <th rowspan="{{count($nilaiDiniyahPraktek)+1}}" style="text-align: center">
            <span>ILMU DINIYAH</span>
    ...
    @foreach($nilaiDiniyahPraktek as $nd)
    <tr>
        <td >{{$loop->iteration}}</td>
        <td style="text-align: left"> {{$nd->mapel->namaMapel}} </td>
        <td>65</td>
    ...
    @endforeach
    @endif
    @if($nilaiBahasaPraktek != null)
    <tr>
        <th rowspan="{{count($nilaiBahasaPraktek)+1}}" style="text-align: center">
            <span>II</span>
        </th>
        <th rowspan="{{count($nilaiBahasaPraktek)+1}}" style="text-align: center">
            <span>ILMU BAHASA</span>
    ...
    @foreach($nilaiBahasaPraktek as $nb)
    <tr>
        <td>{{$loop->iteration}}</td>
        <td style="text-align: left">{{$nb->mapel->namaMapel}}</td>
        <td>65</td>
    ...
    <tr>
        ...
        <th> {{round($nilaiaktif->avg('rataRata'),1)}}</th>
        <th colspan="3" class="subJudul"> {{terbilang(round($nilaiaktif->avg('rataRata'),1))}}</th>
    </tr>
</table>
